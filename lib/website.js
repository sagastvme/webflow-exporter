...
import { createZipFileLocally } from "./zip_helper.js";
import { removeLocalFolders } from "./queues/removeLocalFoldersQueue.js";
...
console.log('showing this before zipping the files')
    const zipBuffer = await createZipFileLocally(localTmpPath);
console.log('finished creating the zip result i got ', zipBuffer)
    
    // job.progress(status_messages.removed_folders())
    // await removeFolders([localTmpPath]);
    console.log('about to send message to the remove queue ', {
      'paths': [localTmpPath]
    })

    await removeLocalFolders.add({
          'paths': [localTmpPath]
        })
console.log('finished sending the message to the remove queue')
    return zipBuffer;
  } catch (error) {
    await removeFolders([localTmpPath]);
    console.log('error in website.js in the catch')
    throw error;
  }
}
