...
const original = fatherUrl
  try {
    makeFolders(paths);
        let first = await getHtml(fatherUrl);
        linksUsed['home'] = generateHtmlName(first)
        linksUsed[original] = generateHtmlName(first)
      ...
      if (href.startsWith("/")) {
        const absoluteUrl = `${baseUrl}${href.replace(/^\//, "")}`;
          ...
          return absoluteUrl;
                ...
                console.log('element new point = ', $(element).attr("href")?.trim())
      }
      return null;
    })
    .get()
...
function generateHtmlName($, extension = '.html') {
