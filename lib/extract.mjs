        ...
        if (src.includes('js/webflow')) {
            if (!fs.existsSync(jsFile)) {
                try {
                    ...
                    const scriptContentRaw = await response.text();

                    // Remove the specific call if needed
                    let scriptContent = scriptContentRaw.replace('brandElement = brandElement || createBadge();', '');
                    
                    // Use a regex to remove the function definition for c()
                    // This regex looks for "function c() {" followed by any characters (including newlines) non-greedily until the first closing brace "}"
                    scriptContent = scriptContent.replace(/function\s+c\(\)\s*{[\s\S]*?}/, '');
